...
$groupVarsAllMain = str_replace("max_journal_size: 512M", "max_journal_size: 512M \n\nwp_cli_packages:\n  - aaemnnosttv/wp-cli-login-command", $groupVarsAllMain);
...
$vagrantfile = str_replace("config.vm.synced_folder ANSIBLE_PATH, '/ansible-nfs', type: 'nfs'", "config.vm.synced_folder ANSIBLE_PATH, '/ansible-nfs', type: 'rsync'", $vagrantfile);
...
$groupVarsAllHelpers = str_replace("wordpress_env_defaults:", "wordpress_env_defaults:\n  acorn_enable_expirimental_router: true", $groupVarsAllHelpers);
...
//
// Replace: `local_path: ../site # path targeting local Bedrock site directory (relative to Ansible root)` with `local_path: ..`
...
foreach ($wordpressSites as $file) {
    ...
    $content = str_replace("local_path: ../site # path targeting local Bedrock site directory (relative to Ansible root)", "local_path: ..\n    public_path: public\n    upload_path: content/uploads", $content);
    $content = preg_replace("/\s+repo_subtree_path: site # relative path to your Bedrock\/WP directory in your repo/", '', $content);
...
foreach ($wordpressSitesAndVaults as $file) {
    ...
    if ($file === 'group_vars/development/wordpress_sites.yml') {
        $content = str_replace('example.test', str_replace('.' . pathinfo($domain, PATHINFO_EXTENSION), '', $domain) . '.test', $content);
    }
